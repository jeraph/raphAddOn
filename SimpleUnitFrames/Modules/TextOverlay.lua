...
local STATIC_STYLES = {
	...
	["HPcurrmax"] = {
		...
		fn = function(unit)
			local curr, max = LCMH1:GetUnitHealth(unit)
			return ("%d/%d"):format(abbrev(curr), abbrev(max))
		end,
	},
	["HPcomplete"] = {
		type = "HP",
		...
		fn = function(unit)
			...
			if max == 100 then -- Only return % when the exact HP is not known.
				...
				return ("%d/%d (%d%%)"):format(abbrev(curr), abbrev(max), percent)
			end
		end,
	},
	["HPnone"] = {
	...
	["MPcurrmax"] = {
		...
		fn = function(unit)
			local curr, max = UnitPower(unit), UnitPowerMax(unit)
			return max > 0 and ("%d/%d"):format(abbrev(curr), abbrev(max)) or "-"
		end,
	},
	["MPcomplete"] = {
		type = "MP",
		...
		fn = function(unit)
			local curr, max = UnitPower(unit), UnitPowerMax(unit)
			return max > 0 and ("%d/%d (%d%%)"):format(abbrev(curr), abbrev(max), curr * 100 / max) or "-"
		end,
	},
	["MPnone"] = {
		type = "MP",
